...
PROFILE_CHECKPOINT_FILE="$TEST_STATE/checkpoint"
...
assert_symlink() {
    ...
    if [[ -L "$filepath" ]]; then
        # Resolve actual symlink target to canonical absolute path
        ...
        # Resolve expected target to canonical absolute path
